...
> Статус: активная разработка, шаг 3 (установка UI-библиотек)
...
| #   | Этап                            | Результат                                                                                                                     |
| --- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1   | Аудит двух репозиториев         | `[OK]` ui-extractor оценён 9/10, UI-Design-Extractor — 6/10                                                                   |
| 2   | Функциональное сравнение        | `[OK]` Определены 19 категорий сравнения, ui-extractor выигрывает в 12/19                                                     |
| 3   | Стратегия миграции              | `[OK]` Выбрана архитектура монорепо с 4 пакетами                                                                              |
| 4   | Выбор UI-основы                 | `[OK]` Изначально предложен shadcn/ui, затем исправлено на Untitled UI FREE                                                   |
| 5   | Фикс финального стека           | `[OK]` Зафиксирован стек из 10+ библиотек                                                                                     |
| 6   | HTML-редизайн                   | `[OK]` Собран полноценный wireframe на 7 экранов                                                                              |
| 7   | Создание unweave                | `[OK]` Репозиторий создан на базе шаблона webstorm-config                                                                     |
| 8   | Настройка монорепо              | `[OK]` 4 пакета: core, cli, mcp, web                                                                                          |
| 9   | Smoke-test CLI                  | `[OK]` Pipeline отработал: Extract -> Analyze -> Spec -> Generate                                                             |
| 10  | Фикс багов в core               | `[OK]` 4 бага закрыты (SVG className, data-testid, JSDoc, CLI path)                                                           |
| 11  | Коммиты и push                  | `[OK]` 3 коммита по Conventional Commits, push успешен                                                                        |
| 12  | Установка зависимостей web      | `[OK]` Добавлены: @tanstack/react-query, zustand, next-themes, cmdk, tailwindcss, postcss, autoprefixer, @tailwindcss/postcss |
| 13  | Миграция eslint-plugin-markdown | `[OK]` Завершена: eslint-plugin-markdown -> @eslint/markdown@8.0.3                                                            |
...
| ---------------- | --------------------------- | ----------------------------------------------------------------------- | -------- |
| База компонентов | `Untitled UI FREE`          | Кнопки, инпуты, селекты, модалки, бейджи, аватары, тултипы + React Aria | `[TODO]` |
| Wow-эффекты      | `Magic UI`                  | Bento-grid, marquee, анимированные карточки для дашборда                | `[TODO]` |
| Дашборд-метрики  | `Tremor Raw`                | Спарклайны, bar/line чарты, KPI-карточки                                | `[TODO]` |
| Command Palette  | `cmdk`                      | Cmd+K палитра                                                           | `[OK]`   |
| Анимации         | `Motion` (ex-Framer Motion) | Переходы шагов wizard, анимации списков                                 | `[TODO]` |
| State (client)   | `Zustand`                   | Клиентское состояние                                                    | `[OK]`   |
| State (server)   | `TanStack Query v5`         | API pipeline, кеширование                                               | `[OK]`   |
| URL-state        | `Nuqs`                      | Типизированные search params                                            | `[TODO]` |
| Формы            | `React Hook Form + Zod`     | Валидация форм извлечения                                               | `[TODO]` |
| Тосты            | `Sonner`                    | Уведомления                                                             | `[TODO]` |
| Иконки           | `Lucide-react`              | Иконки интерфейса                                                       | `[TODO]` |
| Утилиты          | `clsx + tailwind-merge`     | Функция cn() для классов                                                | `[TODO]` |
| Таблицы          | `TanStack Table`            | Таблица компонентов                                                     | `[TODO]` |
| Headless         | `Radix UI`                  | Модалки, диалоги (через Untitled UI)                                    | `[OK]`   |
...
| `Tailwind CSS`        | 4.3    | Утилитарные стили                                |
| `ESLint`              | 10     | Линтинг (миграция на @eslint/markdown завершена) |
...
- Grid 4 колонки: карточки с превью-градиентом, тегами, рейтингом (звезды), кнопкой "Использовать"
...
- Хоткеи: Cmd+N (extract), Cmd+E (export), Esc (закрыть)
...
1. `[TODO]` Установить UI-библиотеки: Untitled UI FREE, Magic UI, Tremor Raw, Motion, Sonner, Lucide
2. `[TODO]` Настроить тему: `globals.css` с токенами из wireframe, ThemeProvider
3. `[TODO]` Реализовать каркас: sidebar + topbar + Cmd+K
4. `[TODO]` Реализовать Dashboard (hero + stats + project cards)
5. `[TODO]` Реализовать Extract Wizard (4 шага)
6. `[TODO]` Реализовать Workspace (split-view + live preview)
7. `[TODO]` Подключить API-роуты к core-функциям
8. `[TODO]` Полировка: анимации, тосты, responsive
...
- 7 экранов: Dashboard, Extract Wizard (4 шага), Workspace, Tokens, References, Cmd+K, Export
