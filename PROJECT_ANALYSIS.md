...
### Этап 5: Незакрытые задачи по итогам сессии стабилизации (24.08.2026 — В очереди [ ])

Зафиксированные проблемы, обнаруженные при отладке жизненного цикла задач, дашборда
и Workspace. Часть исправлений уже вошла в коммиты `6f837ec`, `4d54a45`, `dd5c3d5`,
`2d0f192` (группировка проектов, гонка воркера, гранулярный прогресс, семантическая
группировка компонентов).

1. `[ ]` **Редизайн экрана прогресса Extract Wizard (UX Flow)**:
   - Зафиксированные решения (brainstorm от 24.08.2026):
     - Модалка отвечает только за прогресс; после завершения — немедленный переход на `/workspace` без 2-секундной задержки (вариант B).
     - При ошибке — кнопка `Try Again` с повторным запуском тех же настроек + ссылка `Change settings` на шаг 1.
     - Панель Worker Log остаётся свёрнутой по умолчанию (тонкая строка `View worker log (N)`).
   - `[ ]` Реализовать переход на `/workspace` сразу после статуса `completed`.
   - `[ ]` Реализовать `Try Again` / `Change settings` на терминальном состоянии ошибки.
   - `[ ]` Полный визуальный редизайн степпера (заменить текстовые `[OK]/(...)` маркеры) — после стабилизации логики.
2. `[ ]` **Дашборд: реальное значение метрики Design Tokens**:
   - Карточка статистики «Design Tokens» захардкожена (`value={0}`, данные спарклайна `[0,0,0,0]`) в `packages/web/app/page.tsx`.
   - Подсчитывать сумму токенов завершённых джоб (источник: `result.analysis.designSystem.colors.all.length` и аналогичные шкалы).
3. `[ ]` **Единый test-runner в корне монорепозитория**:
   - Корневой `npx vitest run` падает: нет резолва алиасов `@/*` для пакетов web/core.
   - Настроить `vitest.workspace.ts` (или алиасы в корневом конфиге), чтобы прогон всех тестов одной командой был корректным.
4. `[ ]` **Именование компонентов для хэш-классов без суффикса**:
   - Классы вида `sc-KOGVz` (без `_semanticSuffix`) дают имя `ScKOGVz`.
   - Добавить эвристики: использовать текстовое содержимое/роль/соседний heading для генерации имени.
5. `[ ]` **Прогресс между стадиями analyze/spec/generate**:
   - Прогресс прыгает 33% → 80%: быстрые стадии не успевают отчитаться промежуточными событиями.
   - Опционально: события из analyze/spec или интерполяция на стороне UI.
6. `[ ]` **Технический долг typecheck пакета core**:
   - `tsc --checkJs` выдаёт pre-existing ошибки: отсутствие `@types/node` (process/fs/promises/path), глобальный тип `Page` Playwright, устаревший `--moduleResolution node`.
   - Добавить `@types/node`, типизировать параметры Playwright, обновить флаги tsconfig.
