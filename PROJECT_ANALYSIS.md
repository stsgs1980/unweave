...
| **Библиотека референсов**      | [IN_PROGRESS] | Страница `/references` с каталогом; синхронизация файлов `references/` с БД Prisma реализована в `/api/references`. Осталась кнопка «Использовать референс» (Этап 5). |
| **Export Modal & ZIP**         |    [TODO]     | Генератор ZIP-архивов с компонентами, токенами и конфигурацией (Этап 5).                                                                                              |
| **Command Palette (`⌘K`)**     |     [OK]      | Базовая реализация `CommandPalette.tsx` подключена в глобальном Layout; запланирована индексация компонентов проекта (Этап 5).                                        |
...
### 4.5. Таймауты при выборке проектов и передача строк Viewport в Playwright

- **Проблема**: Запрос `prisma.job.findMany` загружал поле `result` (до 30 МБ на задачу), из-за чего выборка 12 проектов занимала >60 сек и приводила к ошибке `Failed to fetch`. Кроме того, передача строкового Viewport (`"desktop"`) в `browser.newContext()` вызывала сбой воркера.
- **Решение**:
  - В `/api/projects` подсчёт количества компонентов и токенов вынесен в SQL-запрос (`$queryRaw`) без загрузки тяжёлого поля `result`.
  - В `packages/core/src/extract.js` добавлена функция `normalizeViewport`, преобразующая строки в объекты `{ width, height }`.

### 4.6. Гонка worker exit и «осиротевшие» ошибки у завершённых задач (24.08.2026)

- **Проблема**: Флаг `terminalStatusSent` выставлялся после `await saveJobScreenshots()`; воркер успевал завершиться, exit-обработчик помечал джобу `failed` («Worker finished without reporting a result»), затем дописывался `completed` с осиротевшей ошибкой.
- **Решение**:
  - Логика жизненного цикла вынесена в тестируемый модуль `lib/worker-lifecycle.ts` (8 unit-тестов): терминальный флаг ставится синхронно до await, exit-обработчик перечитывает статус из БД, `completed` очищает `error`.
  - Диагностика в тексте ошибки: `(last message: progress N%)`.

### 4.7. Шум и дубли в списках проектов и компонентов (24.08.2026)

- **Проблема**: Дашборд показывал каждый запуск сайта отдельной карточкой; дерево компонентов Workspace выводило сырые теги (`ul`, `button`) с дубликатами по полной строке классов и выдуманными счётчиками вариантов.
- **Решение**:
  - Группировка карточек по нормализованному URL с раскрытием истории прогонов (`project-grouping.ts`).
  - В ядре: группировка по `тег + базовый класс`, имя из семантики класса (`deriveComponentName`), реальные варианты = различные сигнатуры computed-стилей; гранулярный прогресс extract-фазы (7 подшагов, диапазон 5–35%).
...
### Этап 4: Качество данных, UX-flow и инфраструктура разработки (P1 — В очереди [ ])

> Переупорядочено 24.08.2026 по итогам аудита: сначала быстрые победы, влияющие на
> корректность данных и удобство проверки, затем крупные фичи.

1. `[ ]` **Extract Wizard — финализация UX Flow (решения зафиксированы)**:
   - `[ ]` Убрать задержку `COMPLETION_DELAY_MS=2000` в `useExtractionPolling.ts`: переход на `/workspace` сразу после статуса `completed`.
   - `[ ]` Терминальная ошибка: кнопка `Try Again` (перезапуск тех же настроек) + `Change settings` на шаг 1.
   - `[ ]` Панель Worker Log свёрнута по умолчанию (тонкая строка `View worker log (N)`).
   - `[ ]` Визуальный редизайн степпера (заменить текстовые `[OK]/(...)` маркеры) — последним, после стабилизации логики.
2. `[ ]` **Дашборд: реальное значение метрики Design Tokens**:
   - Карточка «Design Tokens» захардкожена (`value={0}`, спарклайн `[0,0,0,0]`) в `packages/web/app/page.tsx`.
   - Источник: сумма `result.analysis.designSystem.colors.all.length` (+ spacing/typography) по завершённым джобам через агрегирующий SQL/API.
3. `[ ]` **Единый test-runner в корне монорепозитория** (быстрый фикс):
   - Корневой `npx vitest run` падает: нет резолва алиасов `@/*` для пакетов web/core.
   - Настроить `vitest.workspace.ts` / алиасы, добавить `npm test` в корень для прогона всех пакетов одной командой.
4. `[ ]` **Именование компонентов для хэш-классов без суффикса**:
   - Классы вида `sc-KOGVz` дают имя `ScKOGVz`.
   - Эвристики: текстовое содержимое, ARIA-role, ближайший heading.
5. `[ ]` **Технический долг typecheck пакета core**:
   - Pre-existing ошибки `tsc --checkJs`: нет `@types/node`, глобальный тип `Page` Playwright, устаревший `--moduleResolution node`.
   - Добавить `@types/node`, типизировать параметры Playwright, обновить tsconfig.

### Этап 5: Экспорт, библиотека референсов и интеграция (P2 — В очереди [ ])

1. `[ ]` **Модальное окно экспорта (Export Modal `⌘E`)**:
   - Клиентский/серверный упаковщик ZIP (`jszip` / `archiver`): файлы компонентов, токены, конфигурация Tailwind.
2. `[ ]` **Библиотека референсов — завершение**:
   - `[x]` Синхронизация локальных файлов `references/` с БД Prisma (уже реализована в `/api/references`: мерж локальных файлов и `prisma.reference.findMany`) — отмечено по аудиту 24.08.2026.
   - `[ ]` Кнопка «Использовать референс» для генерации нового UI в заданной стилистике.
3. `[ ]` **Улучшение Command Palette (`⌘K`)**:
   - Индексация компонентов текущего проекта и переключение между ними с клавиатуры (сейчас — только статические пункты).

### Этап 6: Прогресс пайплайна, тестирование и верификация (P2/P3 — В очереди [ ])

1. `[x]` **Юнит-тесты** — базово выполнено (аудит 24.08.2026):
   - `[x]` Web: 50 тестов (`vitest` + Testing Library): jobStore-хелперы, worker-lifecycle, валидация URL, сторы, UI-компоненты.
   - `[x]` Core: 20 тестов: pipeline timing/progress, классификация компонентов, извлечение элементов.
2. `[ ]` **E2E-тесты полного цикла** (`playwright`): запуск извлечения → статус → Workspace (сейчас проверяется только вручную).
3. `[ ]` **Прогресс между стадиями analyze/spec/generate**: скачок 33% → 80%; события из analyze/spec или интерполяция в UI (низкий приоритет — косметика).
4. `[ ]` **Нагрузочное тестирование опроса статусов при нескольких параллельных воркерах**.
